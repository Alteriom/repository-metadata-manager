...
describe('Feature Managers', () => {
  ...
  describe('HealthScoreManager', () => {
    ...
    it('should calculate health score correctly', async () => {
      ...
      const result = await healthScoreManager.calculateHealthScore('owner', 'repo', categories);
  ...
  describe('DocumentationManager', () => {
    ...
    it('should audit documentation and return score', async () => {
      mockOctokit.rest.repos.getContent
        .mockResolvedValueOnce({ data: { content: Buffer.from('# Test README').toString('base64') } })
        .mockRejectedValueOnce(new Error('Not found'));

      const result = await documentationManager.auditDocumentation('owner', 'repo');
    ...
    it('should analyze document content correctly', async () => {
      const mockContent = {
        data: { content: Buffer.from('# Test Project\n\nThis is a description.').toString('base64') }
      };
      
      mockOctokit.rest.repos.getContent.mockResolvedValue(mockContent);

      const analysis = await documentationManager.analyzeDocument('owner', 'repo', 'README.md');
    ...
    it('should handle missing documents', async () => {
      ...
      const analysis = await documentationManager.analyzeDocument('owner', 'repo', 'MISSING.md');
    ...
    it('should validate README content', async () => {
      const goodContent = '# Project\n\nDescription here\n\n## Installation\n\n## Usage\n\n## Contributing\n\n## License';
      const contentObject = {
        content: Buffer.from(goodContent).toString('base64')
      };
      const validation = await documentationManager.validateReadme(contentObject);
  ...
  describe('SecurityManager', () => {
    ...
    it('should check for security policy', async () => {
      mockOctokit.rest.repos.getContent.mockResolvedValue({
        data: { content: Buffer.from('# Security Policy\n\nReport vulnerabilities...').toString('base64') }
      });

      const result = await securityManager.checkSecurityPolicy('owner', 'repo');
  ...
  describe('BranchProtectionManager', () => {
    ...
    it('should audit branch protection', async () => {
      ...
      const result = await branchProtectionManager.auditBranchProtection('owner', 'repo');
    ...
    it('should handle unprotected branches', async () => {
      mockOctokit.rest.repos.getBranchProtection.mockRejectedValue(new Error('Branch not protected'));
    ...
    it('should check specific protection rules', async () => {
      ...
      const result = branchProtectionManager.checkProtectionRules(protectionData);
  ...
  describe('CICDManager', () => {
      ...
      mockOctokit.rest.actions.listRepoWorkflows.mockResolvedValue({
        data: {
          workflows: [
            ...
            { name: 'Deploy', state: 'active', path: '.github/workflows/deploy.yml' }
          ]
        }
      });
    ...
    it('should analyze workflow quality', async () => {
      const workflows = [
        { name: 'CI', state: 'active', path: '.github/workflows/ci.yml' },
        { name: 'Security', state: 'active', path: '.github/workflows/security.yml' },
