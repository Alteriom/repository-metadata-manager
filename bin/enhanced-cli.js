...
// ASCII Art Banner
console.log(chalk.cyan(figlet.textSync('Repo Manager', { horizontalLayout: 'full' })));
...
program
  ...
  .action(async (options) => {
    ...
    if (options.enforce || options.fix) {
      ...
      console.log(chalk.green(`✅ Applied ${results.fixes.length} security fixes`));
...
program
  ...
  .action(async (options) => {
    ...
    if (options.enforce) {
      ...
      const results = await branchManager.enforceBranchProtection(settings);
...
program
  ...
  .action(async (options) => {
    ...
    if (options.generate) {
      ...
      console.log(chalk.green(`✅ Generated ${results.generated.length} documentation files`));
...
program
  ...
  .option('--generate <type>', 'Generate workflow templates (node|security|release|all)')
  .action(async (options) => {
    const config = await loadConfig();
    ...
    if (options.generate) {
      console.log(chalk.blue(`🔧 Generating ${options.generate} workflow templates...\n`));
      const generated = await cicdManager.generateWorkflowTemplates(options.generate);
      console.log(chalk.green(`✅ Generated ${generated.length} workflow files`));
...
function displayHealthSummary(health) {
  ...
  for (const [category, data] of Object.entries(health.categories)) {
    const color = data.score >= 80 ? chalk.green : data.score >= 60 ? chalk.yellow : chalk.red;
...
function displayDocumentationResults(results) {
  ...
  results.files.forEach(file => {
    ...
    if (file.issues.length > 0) {
      file.issues.forEach(issue => console.log(chalk.gray(`    - ${issue}`)));
    }
  });
}
...
function displayCICDResults(results) {
  ...
  results.workflows.forEach(workflow => {
    ...
    if (workflow.securityIssues.length > 0) {
      ...
      workflow.securityIssues.forEach(issue => console.log(`    - ${issue}`));
    }
    console.log('');
  });
}
...
async function applyAutomaticFixes(config) {
  ...
  try {
    ...
    // Documentation fixes
    const docsManager = new DocumentationManager(config);
    const docResults = await docsManager.generateMissingDocs({ autoGenerate: true });
