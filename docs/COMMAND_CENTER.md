...
Dependabot or any context without App secrets runs a secret-free local checker scope covering CI/CD, dependencies, documentation, IoT applicability, licensing, and security. A non-Dependabot restricted context publishes `Compliance Check (restricted)` through its built-in token; Dependabot retains the enforced workflow result, logs, and report artifact but skips Checks API publication because GitHub makes its token read-only. Restricted feedback cannot satisfy the protected `Compliance Check` context. A maintainer can request the trusted, default-branch workflow for an open PR with `gh api repos/OWNER/REPOSITORY/dispatches -f event_type=repository-compliance -F 'client_payload[pull_request_number]=123'`. The dispatcher resolves the PR's current immutable merge SHA for evaluation and publishes the App-sourced result on its head SHA. Authoritative branch-protection and repository-metadata verification, the protected App-sourced checks, and PR report comments require the App-backed context.

The same default-branch workflow runs candidate code in a separate secret-free job. It replaces candidate tests with the protected branch's tests, uses the protected lint configuration and test runner, disables lifecycle scripts, and exposes no App credentials. Before execution, it copies both trees to a root-owned, non-writable sandbox under `/opt` and gives the worker identity only a private writable home directory; this avoids relying on hosted-runner home-directory permissions or a shared Jest transform cache. On the hosted Linux runner, the protected supervisor and Jest controller run as root while every Jest worker is dropped to the unprivileged `nobody` UID/GID; Node's SIGUSR1 inspector activation is disabled. Candidate workers therefore cannot signal, trace, or attach a debugger to the controller that owns result authority. Worker preloads also replace and freeze both inspector-module APIs and reject the underlying inspector binding before candidate code loads, preventing same-process debugger attachment. The supervisor first records the exact test-file/name multiset from a clean baseline run and freezes Jest registration/assertion globals before candidate modules load. The symbol-keyed Jest matcher map and custom-equality registry are installed as non-writable properties; public matcher-extension APIs are blocked; and the standard intrinsic constructors, namespaces, prototypes, and global bindings used by matchers are recursively frozen. A preload gives each worker a random authentication key through a one-use pipe that is read and closed before candidate code. Before candidate modules load, it captures private bindings to the native serializer, HMAC operations, timing comparison, and base64 encoder. Controller requests and worker responses use direction-bound HMAC envelopes and unique request IDs; responses are accepted only for outstanding authenticated requests, and terminal IDs cannot be replayed. Candidate-visible `process.emit` cannot inject local worker requests, the cached trusted `testWorker` export is frozen, raw descriptors are hidden from both `process.channel` aliases, and candidate-visible `process.send` remains outside the result protocol. A protected reporter in the separate Jest controller sends the authoritative result directly to the supervisor over parent IPC, with no candidate-visible result-file path. The candidate result must contain the identical identities with every suite and assertion passed. Premature termination, exit-handler forgery, matcher or intrinsic forgery, inspector attachment, worker request/result injection, raw-fd injection, serializer/cryptographic monkeypatching, replaced globals, dummy tests, skipped assertions, missing output, replayed or duplicate authority messages all fail the job. A different job receives only that job's GitHub result and publishes `Trusted Test & Lint` with the dedicated App. Candidate-defined `Test & Lint` and `Security Summary` workflows remain useful feedback, but they are not merge-authority controls because a pull request can edit their workflow definitions.

Both trusted jobs check out the evaluator, policy, tests, and test supervisor from the exact 40-character commit stored in the repository variable `REPOSITORY_MANAGER_CONTROL_SHA`; branch names and event SHAs are rejected as control-plane selectors. The App-backed workflow has no `push`, `schedule`, or `workflow_dispatch` trigger, so merged candidate code cannot receive credentials automatically. For every pull request containing a non-documentation change, including executable code, manifests, configuration, or workflow changes, the default-branch resolver fails before secrets are loaded unless a repository administrator comments `/approve-command-center HEAD_SHA` on that exact pull-request head. The resolver fetches the Git trees for the captured base and merge commit SHAs, then classifies every path whose immutable tree entry differs. If either recursive tree response is truncated or otherwise incomplete, classification fails closed by requiring administrator review. Renamed files appear as a deletion and an addition, so both paths are evaluated. The exemption is limited to documentation assets under `docs/`, conventional root documents, and root license/notice files; known agent-control and dependency-manifest names such as `AGENTS.md`, `SKILL.md`, `requirements.txt`, and `CMakeLists.txt` always require attestation. A later push changes the SHA and invalidates the attestation. This preserves a trusted human review action for code while avoiding the solo maintainer's self-approval deadlock. Update the pinned SHA only after reviewing and releasing a new control-plane commit, and record both actions as command-center audit events.

Trusted reruns and scheduled command-center audits use only `repository_dispatch`, whose workflow definition is always loaded from the default branch and whose payload must identify an open pull request targeting this repository's protected `main` branch. The App-backed workflow intentionally has no `workflow_dispatch` trigger because GitHub permits manual dispatch against a collaborator-controlled branch ref.
...
- require an exact-head `/approve-command-center HEAD_SHA` administrator attestation for every non-documentation change, and pin `REPOSITORY_MANAGER_CONTROL_SHA` to an audited immutable commit;
