  ...
  async checkSecurityPolicy(results) {
    ...
    if (exists) {
      ...
      const requiredSections = [
        { name: 'Supported Versions', pattern: /supported\s+versions/i },
        { name: 'Reporting Instructions', pattern: /reporting\s+a\s+vulnerability/i },
    ...
    results.checks.push({
      ...
      weight: 30,
      issues: issues,
      fix: exists ? null : 'Create SECURITY.md file with vulnerability reporting instructions'
    });
  }
  ...
  async checkDependencies(results) {
    ...
    if (fs.existsSync(packageFile)) {
      try {
        ...
        const hasSecurityDeps = securityPackages.some(pkg => deps[pkg]);
        if (hasSecurityDeps) {
          score += 10;
        }
    ...
    results.checks.push({
      ...
      weight: 25,
      issues: issues,
      fix: issues.length > 0 ? 'Review and update dependency management' : null
    });
  }
