...
class DocumentationTokenTester {
  constructor() {
    ...
    this.repo = process.env.GITHUB_REPOSITORY_NAME || 'repository-metadata-manager';
  ...
  async testAllTokens() {
    console.log(chalk.blue('🔍 Testing all tokens for documentation access...'));
  ...
  async testToken(tokenName, tokenValue) {
    ...
    for (const endpoint of this.docEndpoints) {
      ...
      try {
        ...
        const status = response.status === 200 ? '✅' : `⚠️ ${response.status}`;
    ...
    results.successRate = Math.round((results.successful.length / results.totalAttempts) * 100);
    
    console.log(chalk.bold(`\n📊 ${tokenName} Results:`));
    console.log(`  Success Rate: ${results.successRate}% (${results.successful.length}/${results.totalAttempts})`);
  ...
  summarizeResults(results) {
    ...
    console.log(chalk.bold('\n📊 Success Rates:'));
    Object.values(results).forEach(result => {
      const color = result.successRate >= 80 ? 'green' : 
                   result.successRate >= 60 ? 'yellow' : 'red';
      const badge = result.tokenName === bestToken ? ' 🏆' : '';
      console.log(`  ${chalk[color](result.tokenName)}: ${result.successRate}%${badge}`);
    });
    ...
    if (bestScore === 100) {
      console.log('  🎉 Perfect! Use ' + bestToken + ' for all documentation operations');
    } else if (bestScore >= 80) {
      console.log('  ✅ Use ' + bestToken + ' as primary, with local fallback for failed endpoints');
    } else if (bestScore >= 50) {
      console.log('  ⚠️  Use ' + bestToken + ' with extensive local fallback');
    } else {
      console.log('  🚨 Use local file system only - all tokens have issues');
    }
    ...
    const permissionErrors = allFailures.filter(f => f.error.includes('Permission') || f.error.includes('403'));
    
    if (serverErrors.length > 0) {
      console.log('  🔧 GitHub API has server issues - local fallback recommended');
    }
    if (permissionErrors.length > 0) {
      console.log('  🔑 Some tokens lack required permissions - check token scopes');
    }
  }
  ...
  async testSpecificEndpoint(endpoint, tokenName) {
    ...
    try {
      ...
      console.log(`📄 Type: ${Array.isArray(response.data) ? 'Directory' : 'File'}`);
...
if (require.main === module) {
  ...
  if (args.length === 2) {
    ...
    tester.testSpecificEndpoint(endpoint, tokenName)
      .catch(error => process.exit(1));
  } else {
    // Test all tokens
    tester.testAllTokens()
      .catch(error => {
