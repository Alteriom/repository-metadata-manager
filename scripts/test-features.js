...
console.log(
    chalk.cyan(figlet.textSync('Repo Test', { horizontalLayout: 'full' }))
);
...
async function getRealDocumentationScore() {
    try {
        ...
        console.log(
            chalk.yellow(
                `⚠️ Could not get real documentation score: ${error.message}`
            )
        );
...
async function getRealSecurityScore() {
    try {
        ...
        console.log(
            chalk.yellow(
                `⚠️ Could not get real security score: ${error.message}`
            )
        );
...
async function getRealBranchProtectionScore() {
    try {
        ...
        console.log(
            chalk.yellow(
                `⚠️ Could not get real branch protection score: ${error.message}`
            )
        );
...
async function testDocumentationLocal() {
    console.log(
        chalk.blue('📚 Testing Documentation Analysis (Local Mode)...\n')
    );
...
function analyzeReadme(content) {
    ...
    const checks = [
        { name: 'Has title', pattern: /^#\s+.+/m, points: 2 },
        {
            ...
            pattern: /description|purpose|what/i,
            points: 2,
        },
        {
...
function analyzeChangelog(content) {
    ...
    if (
        /### (Added|Changed|Deprecated|Removed|Fixed|Security)/i.test(content)
    ) {
        analysis.score += 5;
    } else {
...
async function testHealthScore() {
    ...
    for (const [category, score] of Object.entries(categories)) {
        ...
        console.log(
            `  ${icon} ${category}: ${color(score + '%')} (weight: ${weight}%)`
        );
    }
