...
class LocalCICDAuditor {
    ...
    categorizeWorkflows(workflowFiles) {
        return {
            ...
            security: workflowFiles.some((f) =>
                /security|codeql|dependency/i.test(f)
            ),
            release: workflowFiles.some((f) =>
                /release|deploy|publish/i.test(f)
            ),
        };
    }
            ...
            try {
                ...
                essentialScripts.forEach(({ name, weight }) => {
                    if (scripts[name]) {
                        ...
                        recommendations.push(
                            `Add '${name}' script to package.json`
                        );
                    }
                });
        ...
        if (fs.existsSync(packageJsonPath)) {
            try {
                ...
                const hasTestFramework = testFrameworks.some(
                    (framework) => devDeps[framework]
                );
                if (hasTestFramework) {
                    score += 8;
                } else {
                    issues.push(
                        'No testing framework found in devDependencies'
                    );
                    recommendations.push(
                        'Add a testing framework (Jest, Mocha, etc.)'
                    );
                }
    ...
    async checkAutomation(results) {
        ...
        if (
            fs.existsSync(path.join(this.basePath, '.github', 'dependabot.yml'))
        ) {
            score += 5;
        } else {
            recommendations.push(
        ...
        if (fs.existsSync(templatesPath)) {
            ...
            if (
                templates.some((t) => t.name.includes('PULL_REQUEST_TEMPLATE'))
            ) {
                score += 2;
            }
        }
        ...
        if (fs.existsSync(packageJsonPath)) {
            try {
                ...
                const releaseScripts = Object.keys(scripts).filter(
                    (s) => s.includes('release') || s.includes('version')
                );
    ...
    displayResults(results) {
        ...
        results.checks.forEach((check) => {
            ...
            if (check.workflows && check.workflows.length > 0) {
                check.workflows.forEach((workflow) => {
                    console.log(
                        `   📄 ${workflow.name} (Score: ${workflow.score})`
                    );
        ...
        console.log(
            '2. Implement security scanning (CodeQL, dependency audit)'
        );
