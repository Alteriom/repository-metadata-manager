...
class LocalDocumentationAuditor {
  ...
  async auditDocumentation() {
    ...
    const requiredDocs = [
      ...
      { file: 'docs/', weight: 10, validator: this.validateDocsDirectory.bind(this) }
    ];
  ...
  async validateDocsDirectory(files) {
    ...
    const foundDirs = expectedDirs.filter(dir => existingDirs.includes(dir));
    validation.score += Math.min(foundDirs.length * 2, 6); // Max 6 points for directories

    if (foundDirs.length < expectedDirs.length) {
      validation.recommendations.push('Consider organizing docs into subdirectories: guides, development, releases');
    }
  ...
  displayResults(results) {
    ...
    console.log(`Status: ${getScoreColor(results.score)} ${
    ...
    for (const file of results.files) {
      ...
      const scorePercent = file.weight > 0 ? Math.round((file.score / file.weight) * 100) : 0;
      console.log(`${status} ${file.file.padEnd(35)} ${scorePercent}% (${file.score}/${file.weight})`);
  ...
  suggestImprovements(results) {
    ...
    const incompleteFiles = results.files.filter(f => f.exists && f.score < f.weight);
    ...
    if (results.score < 80) {
      ...
      if (!results.files.find(f => f.file === 'CONTRIBUTING.md')?.exists) {
