...
class LocalDocumentationAuditor {
  ...
  async validateReadme(analysis) {
    if (!analysis.exists || !analysis.content) {
      return { score: 0, issues: ['README.md not found'], recommendations: ['Create README.md'] };
    }
  ...
  async validateChangelog(analysis) {
    if (!analysis.exists || !analysis.content) {
      return { score: 0, issues: ['CHANGELOG.md not found'], recommendations: ['Create CHANGELOG.md'] };
    }
  ...
  async validateContributing(analysis) {
    if (!analysis.exists || !analysis.content) {
      return { score: 0, issues: ['CONTRIBUTING.md not found'], recommendations: ['Create CONTRIBUTING.md'] };
    }
  ...
  async validateCodeOfConduct(analysis) {
    if (!analysis.exists || !analysis.content) {
      return { score: 0, issues: ['CODE_OF_CONDUCT.md not found'], recommendations: ['Create CODE_OF_CONDUCT.md'] };
    }
  ...
  async validateLicense(analysis) {
    if (!analysis.exists || !analysis.content) {
      return { score: 0, issues: ['LICENSE not found'], recommendations: ['Add LICENSE file'] };
    }
  ...
  async validateIssueTemplates(analysis) {
    ...
    if (analysis.exists && Array.isArray(analysis.content)) {
      const templates = analysis.content.filter(file => file.endsWith('.md'));
      ...
      validation.recommendations.push('Create .github/ISSUE_TEMPLATE/ directory with templates');
    }
  ...
  async validatePRTemplate(analysis) {
    if (!analysis.exists || !analysis.content) {
      return { score: 0, issues: ['PR template not found'], recommendations: ['Create .github/PULL_REQUEST_TEMPLATE.md'] };
    }
  ...
  generateRecommendations(files) {
    ...
    files.forEach(file => {
      if (file.issues.length > 0) {
        allRecommendations.push(`🔴 ${file.file}: ${file.issues.join(', ')}`);
      }
  ...
  displayResults(results) {
    ...
    results.files.forEach(file => {
      ...
      console.log(`  ${status} ${file.file} (${file.score}/${file.weight} - ${scorePercent}%)`);
...
if (require.main === module) {
  ...
  auditor.auditDocumentation().catch(error => {
    console.error(chalk.red('❌ Documentation audit failed:'), error.message);
    process.exit(1);
  });
}
