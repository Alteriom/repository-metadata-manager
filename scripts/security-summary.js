...
async function runSecuritySummary() {
    ...
    if (successCount >= 3) {
        ...
        console.log(
            chalk.yellow('Some security checks failed - review required.')
        );
    }
    ...
    try {
        ...
        console.log(
            chalk.green(`✅ Local Security Score: ${localResults.score}/100`)
        );
        ...
        console.log(
            chalk.yellow(`⚠️ Local security check failed: ${error.message}`)
        );
    }
}
