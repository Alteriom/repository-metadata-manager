...
class Policy {
  ...
  static validate(policy) {
    ...
    assertKnownProperties(branch, new Set([
      'requiredApprovals', 'maximumRequiredApprovals', 'requireStatusChecks', 'requireStrictStatusChecks',
    ...
    if (branch.maximumRequiredApprovals !== undefined &&
        branch.maximumRequiredApprovals < (branch.requiredApprovals || 0)) {
      throw new Error('branchProtection.maximumRequiredApprovals must be greater than or equal to requiredApprovals');
    }
    for (const [key, value] of Object.entries(branch)) {
      if (!['requiredApprovals', 'maximumRequiredApprovals'].includes(key) && typeof value !== 'boolean') {
