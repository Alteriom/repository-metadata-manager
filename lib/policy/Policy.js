...
class Policy {
  ...
  static validate(policy) {
    ...
    assertKnownProperties(branch, new Set([
      'requiredApprovals', 'requireStatusChecks', 'requireStrictStatusChecks',
      'requireCodeOwnerReviews', 'requireConversationResolution', 'enforceAdmins',
      'requireSignedCommits', 'requireLinearHistory',
    ]), 'branchProtection');
    if (branch.requiredApprovals !== undefined && (!Number.isInteger(branch.requiredApprovals) || branch.requiredApprovals < 0)) {
      throw new Error('branchProtection.requiredApprovals must be a non-negative integer');
    }
    for (const [key, value] of Object.entries(branch)) {
      if (key !== 'requiredApprovals' && typeof value !== 'boolean') {
