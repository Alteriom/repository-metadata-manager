...
class AutomationManager extends RepositoryManager {
    ...
    async runOrganizationHealthAudit(options = {}) {
        ...
        try {
            ...
            console.log(
                chalk.gray(`Found ${repositories.length} repositories\n`)
            );
            ...
            if (parallel && repositories.length > 1) {
                console.log(
                    chalk.gray(
                        `Processing ${concurrency} repositories in parallel...\n`
                    )
                );
                results.push(
                    ...(await this.auditRepositoriesParallel(
                        repositories,
                        concurrency
                    ))
                );
            ...
            console.error(
                chalk.red(
                    `❌ Organization health audit failed: ${error.message}`
                )
            );
            throw error;
        }
    ...
    async auditSingleRepository(repo) {
        try {
            ...
            console.log(
                chalk.yellow(
                    `⚠️  Could not audit ${repo.name}: ${error.message}`
                )
            );
            return null;
        }
    ...
    async saveHistoricalData(auditResult) {
        try {
            ...
            console.log(
                chalk.yellow(
                    `⚠️  Could not save historical data: ${error.message}`
                )
            );
        }
    }
    ...
    async calculateTrends(currentAudit) {
        try {
            ...
            const unhealthyDiff =
                currentAudit.unhealthyRepos.length -
                previousData.unhealthyCount;

            // Calculate per-repo trends
            const repoTrends = {};
            currentAudit.results.forEach((current) => {
                const previous = previousData.repositories.find(
            ...
            return {
                ...
                unhealthyDiff,
                repoTrends,
                previousDate: previousFile.match(
                    /health-(\d{4}-\d{2}-\d{2})/
                )[1],
                daysAgo: Math.floor(
                    (new Date() - new Date(previousData.timestamp)) /
                        (1000 * 60 * 60 * 24)
                ),
            };
        } catch (error) {
            console.log(
                chalk.yellow(`⚠️  Could not calculate trends: ${error.message}`)
            );
            return null;
        }
    }
    ...
    displayTrends(trends) {
        ...
        console.log(
            chalk.gray(
                `Comparing with data from ${trends.daysAgo} days ago (${trends.previousDate})\n`
            )
        );

        // Overall trends
        const scoreIndicator =
            trends.avgScoreDiff > 0
                ? '📈'
                : trends.avgScoreDiff < 0
                  ? '📉'
                  : '➡️';
        const scoreColor =
            trends.avgScoreDiff > 0
                ...
                : trends.avgScoreDiff < 0
                  ...
                  : chalk.gray;
        console.log(
            `${scoreIndicator} Average Score: ${scoreColor(trends.avgScoreDiff > 0 ? '+' : '')}${scoreColor(trends.avgScoreDiff.toFixed(1))} points`
        );

        const unhealthyIndicator =
            trends.unhealthyDiff < 0
                ? '✅'
                : trends.unhealthyDiff > 0
                  ? '⚠️'
                  : '➡️';
        const unhealthyColor =
            trends.unhealthyDiff < 0
                ? chalk.green
                : trends.unhealthyDiff > 0
                  ? chalk.red
                  : chalk.gray;
        console.log(
            `${unhealthyIndicator} Unhealthy Repos: ${unhealthyColor(trends.unhealthyDiff > 0 ? '+' : '')}${unhealthyColor(trends.unhealthyDiff)}`
        );

        // Repository-level trends
        const improved = Object.entries(trends.repoTrends).filter(
            ([_, t]) => t.improved
        );
        const declined = Object.entries(trends.repoTrends).filter(
            ([_, t]) => t.declined
        );
        ...
        if (improved.length > 0) {
            ...
            improved.slice(0, 5).forEach(([name, trend]) => {
                console.log(
                    chalk.green(
                        `  ↗ ${name}: +${trend.scoreDiff.toFixed(1)} points`
                    )
                );
            });
            if (improved.length > 5) {
                console.log(
                    chalk.gray(`  ... and ${improved.length - 5} more`)
                );
            }
        }
        ...
        if (declined.length > 0) {
            ...
            if (declined.length > 5) {
                console.log(
                    chalk.gray(`  ... and ${declined.length - 5} more`)
                );
            }
        }
    }
    ...
    async detectMissingWorkflows(options = {}) {
        ...
        try {
            ...
            for (const repo of repositories) {
                try {
                    const repoConfig = {
                        ...this.config,
                        owner:
                            this.config.owner || repo.full_name?.split('/')[0],
                        repo: repo.name,
                    };
                    ...
                    const missing = {
                        ...
                        fullName:
                            repo.full_name ||
                            `${repoConfig.owner}/${repo.name}`,
                        language: repo.language,
                        workflows: workflows.workflows || [],
                        missingCI: !workflows.workflows?.some(
                            (w) =>
                                w.name?.includes('CI') ||
                                w.name?.includes('test')
                        ),
                        missingSecurity: !workflows.workflows?.some(
                            (w) =>
                    ...
                    console.log(
                        chalk.gray(
                            `   Could not analyze ${repo.name}: ${error.message}`
                        )
                    );
                }
            }
            ...
            console.log(
                `Repositories with missing workflows: ${missingWorkflows.length}`
            );
    ...
    async autoFixComplianceIssues(options = {}) {
        ...
        try {
            ...
            for (const repo of repositories) {
                ...
                try {
                    // Check for missing documentation (check remote repos via GitHub API)
                    const missingDocs = await docsManager.checkDocumentation({ remote: true });

                    if (missingDocs.missing && missingDocs.missing.length > 0) {
                        console.log(
                            chalk.yellow(
                                `  ⚠️  Missing: ${missingDocs.missing.join(', ')}`
                            )
                        );

                        if (!dryRun) {
                            // Auto-generate missing docs via GitHub API
                            for (const doc of missingDocs.missing) {
                                console.log(chalk.gray(`  Generating ${doc}...`));
                                const generated =
                                    await docsManager.generateDocumentation(
                                        doc,
                                        { remote: true, branch: 'main' }
                                    );
                                if (generated) {
                                    fixes.push({
                                        type: 'documentation',
                                    ...
                                    console.log(
                                        chalk.green(`  ✓ Generated ${doc}`)
                                    );
                                } else {
                                    console.log(
                                        chalk.red(`  ✗ Failed to generate ${doc}`)
                                    );
                                }
                            }
                        } else {
                    ...
                    console.log(
                        chalk.red(
                            `  ✗ Could not check documentation: ${error.message}`
                        )
                    );
                }
    ...
    async trackDependencies(options = {}) {
        console.log(
            chalk.blue('📦 Tracking Dependencies Across Organization...\n')
        );
        ...
        try {
                ...
                try {
                    ...
                    if (
                        ...
                        try {
                            ...
                            console.log(
                                chalk.gray(`  No package.json in ${repo.name}`)
                            );
                        }
                    }
                } catch (error) {
            ...
            if (conflicts.length > 0 && options.report) {
                ...
                if (conflicts.length > 10) {
                    console.log(
                        chalk.gray(`  ... and ${conflicts.length - 10} more`)
                    );
                }
            }
    ...
    async categorizeRepositories(repositories) {
        ...
        Object.entries(categories).forEach(([category, repos]) => {
            if (repos.length > 0) {
                ...
                console.log(
                    `${icon} ${category.toUpperCase()}: ${repos.length} repos`
                );
    ...
    calculateIssuePriority(issue, repo) {
        ...
        if (
            ...
            issueText.includes('vulnerability')
        ) {
            impact = 10;
            effort = 7;
            category = 'security';
        ...
        // Documentation quality
            ...
            issueText.includes('contributing')
        ) {
            impact = 6;
            effort = 4;
            category = 'documentation';
        ...
        const scoreMultiplier =
            repo.score < 50 ? 1.5 : repo.score < 60 ? 1.2 : 1.0;
    ...
    displayPrioritizedFixes(priorities, options = {}) {
        ...
        console.log(
            chalk.yellow(
                `Found ${priorities.length} repositories needing attention\n`
            )
        );
        ...
        for (const repo of priorities) {
                ...
                const priorityColor =
                    item.priority > 15
                        ...
                        : item.priority > 10
                          ...
                          : chalk.gray;
                const effortIcon =
                    item.effort <= 3 ? '🟢' : item.effort <= 6 ? '🟡' : '🔴';
                console.log(
                    `  ${idx + 1}. ${priorityColor(`[P:${item.priority.toFixed(1)}]`)} ${effortIcon} ${item.issue}`
                );
                console.log(
                    chalk.gray(
                        `     Impact: ${item.impact}/10, Effort: ${item.effort}/10, Category: ${item.category}`
                    )
                );
            });
            ...
            if (repo.issues.length > 3) {
                console.log(
                    chalk.gray(
                        `     ... and ${repo.issues.length - 3} more issues`
                    )
                );
            }
    ...
    displayGroupedIssues(groups) {
        ...
        Object.values(groups)
            ...
            .forEach((group) => {
                ...
                console.log(
                    chalk.gray(
                        `   ${group.count} issues across ${group.repos.length} repositories`
                    )
                );
                console.log(
                    chalk.gray(
                        `   Avg Priority: ${group.avgPriority.toFixed(1)}`
                    )
                );
                console.log(
                    chalk.gray(
                        `   Repos: ${group.repos.slice(0, 5).join(', ')}${group.repos.length > 5 ? '...' : ''}`
                    )
                );
            });
    }
    ...
    displayBatchFixSuggestions(priorities) {
        ...
        suggestions.forEach((suggestion, idx) => {
            ...
            console.log(
                chalk.gray(
                    `   Affects: ${suggestion.affectedRepos} repositories`
                )
            );
    ...
    /**
     * Generate comprehensive compliance report across all repositories
     */
    async generateComplianceReport(options = {}) {
        console.log(
            chalk.blue('📋 Generating Organization Compliance Report...\n')
        );
        ...
        try {
            ...
            console.log(
                chalk.gray(`Analyzing ${repositories.length} repositories...\n`)
            );
            ...
            for (const repo of repositories) {
                try {
                    ...
                    const healthManager = new HealthScoreManager(repoConfig);
                    const healthData =
                        await healthManager.calculateHealthScore();
                    ...
                    const repoReport = {
                        ...
                        score: healthData.score,
                        grade: healthData.grade,
                        compliant: healthData.score >= 80,
                        issues: healthData.criticalIssues || [],
                        categories: healthData.categories,
                    };

                    if (repoReport.compliant) {
                        report.compliantRepos.push(repoReport);
                    } else {
                        report.nonCompliantRepos.push(repoReport);
                    }

                    // Track critical issues
                    if (healthData.categories.security?.score < 70) {
                    ...
                    console.log(
                        `${repoReport.compliant ? chalk.green('✓') : chalk.red('✗')} ${repo.name}: ${healthData.score}/100 (${healthData.grade})`
                    );
                } catch (error) {
                    console.log(
                        chalk.yellow(
                            `⚠️  Could not analyze ${repo.name}: ${error.message}`
                        )
                    );
                }
            }
            ...
            report.summary.avgHealthScore =
                allScores.length > 0
                    ? (
                          allScores.reduce((sum, r) => sum + r.score, 0) /
                          allScores.length
                      ).toFixed(1)
                    : 0;
            ...
            console.log(chalk.blue('\n📊 Compliance Summary:\n'));
            console.log(
                `Total Repositories: ${chalk.cyan(report.totalRepositories)}`
            );
            console.log(
                `Compliant (≥80): ${chalk.green(report.compliantRepos.length)}`
            );
            console.log(
                `Non-Compliant (<80): ${chalk.red(report.nonCompliantRepos.length)}`
            );
            console.log(
                `Average Health Score: ${chalk.cyan(report.summary.avgHealthScore)}`
            );
            console.log(
                `Critical Security Issues: ${chalk.red(report.summary.securityIssues)}`
            );
            ...
            if (report.criticalIssues.length > 0) {
                ...
                report.criticalIssues.forEach((issue) => {
                    console.log(
                        `  - ${issue.repo}: ${chalk.yellow(issue.message)}`
                    );
                });
            }
    ...
    /**
     * Automated repository maintenance across organization
     */
    async performMaintenance(options = {}) {
        console.log(
            chalk.blue('🔧 Performing Automated Repository Maintenance...\n')
        );
        ...
        try {
                ...
                switch (task) {
                    case 'stale-issues':
                        results.summary[task] =
                            await this.checkStaleIssues(repositories, options);
                        break;
                    case 'outdated-deps':
                        results.summary[task] =
                        ...
                        break;
                    case 'unused-workflows':
                        results.summary[task] =
                            await this.checkUnusedWorkflows(
                                repositories,
                                options
                            );
                        break;
                    default:
                        console.log(
                            chalk.yellow(`⚠️  Unknown task: ${task}`)
                        );
                }
            ...
            console.error(
                chalk.red(`❌ Maintenance failed: ${error.message}`)
            );
    ...
    /**
     * Check for stale issues across repositories
     */
    async checkStaleIssues(repositories, options = {}) {
        ...
        console.log(
            chalk.gray(
                `Checking for issues inactive for ${staleDays}+ days...\n`
            )
        );
        ...
        for (const repo of repositories.slice(0, 5)) {
            ...
            try {
                ...
                console.log(chalk.gray(`  Checking ${repo.name}...`));
                ...
                results.repositories.push({
                    name: repo.name,
                    staleCount: 0, // Would calculate from API in real implementation
                });
            } catch (error) {
                console.log(
                    chalk.yellow(
                        `  ⚠️  Could not check ${repo.name}: ${error.message}`
                    )
                );
            }
        }
        ...
        console.log(
            chalk.cyan(`  Found ${results.staleIssues} stale issues\n`)
        );
        return results;
    }
     ...
     */
    async checkOutdatedDependencies(repositories, _options = {}) {
        const results = {
            totalRepos: 0,
            reposWithOutdated: 0,
            criticalUpdates: [],
        ...
        console.log(chalk.gray('Checking for outdated dependencies...\n'));
        ...
        for (const repo of repositories.slice(0, 5)) {
            ...
            try {
                ...
                // In real implementation, would check package.json and run npm outdated
        ...
        console.log(
            chalk.cyan(
                `  Checked ${results.totalRepos} repositories for outdated dependencies\n`
            )
        );
        return results;
    }
    ...
    /**
     * Check for unused or inactive workflows
     */
    async checkUnusedWorkflows(repositories, _options = {}) {
        const results = {
            totalWorkflows: 0,
            unusedWorkflows: [],
            recommendations: [],
        ...
        console.log(chalk.gray('Checking for unused workflows...\n'));
        ...
        for (const repo of repositories.slice(0, 5)) {
            ...
            try {
                ...
                console.log(
                    chalk.gray(
                        `  ${repo.name}: ${workflows.workflows?.length || 0} workflows`
                    )
                );
                results.totalWorkflows += workflows.workflows?.length || 0;

                // In real implementation, would check workflow run history
            } catch (error) {
                console.log(
                    chalk.gray(
                        `  Could not check ${repo.name}: ${error.message}`
                    )
                );
            }
        }
        ...
        console.log(
            chalk.cyan(
                `  Found ${results.totalWorkflows} workflows across repositories\n`
            )
        );
        return results;
    }
    ...
    /**
     * Generate security dashboard with vulnerability insights
     */
    async generateSecurityDashboard(options = {}) {
        console.log(
            chalk.blue('🔒 Generating Security Dashboard...\n')
        );
        ...
        try {
            const repositories =
                await this.multiRepoManager.discoverRepositories();
            dashboard.overview.totalRepos = repositories.length;

            console.log(
                chalk.gray(
                    `Scanning ${repositories.length} repositories for security issues...\n`
                )
            );
            ...
            for (const repo of repositories) {
                try {
                    ...
                    const securityScore =
                        healthData.categories.security?.score || 0;

                    if (securityScore >= 80) {
                        dashboard.overview.secureRepos++;
                        console.log(
                            `${chalk.green('✓')} ${repo.name}: ${securityScore}/100`
                        );
                    } else {
                        dashboard.overview.vulnerableRepos++;
                        console.log(
                            `${chalk.red('✗')} ${repo.name}: ${securityScore}/100`
                        );

                        if (securityScore < 50) {
                            dashboard.overview.criticalVulnerabilities++;
                        } else if (securityScore < 70) {
                            dashboard.overview.highVulnerabilities++;
                        }
                    }
                } catch (error) {
                    console.log(
                        chalk.yellow(
                            `⚠️  Could not scan ${repo.name}: ${error.message}`
                        )
                    );
                }
            }
            ...
            console.log(chalk.blue('\n🔒 Security Overview:\n'));
            console.log(
                `Total Repositories: ${chalk.cyan(dashboard.overview.totalRepos)}`
            );
            console.log(
                `Secure (≥80): ${chalk.green(dashboard.overview.secureRepos)}`
            );
            console.log(
                `Vulnerable (<80): ${chalk.red(dashboard.overview.vulnerableRepos)}`
            );
            console.log(
                `Critical Issues (<50): ${chalk.red(dashboard.overview.criticalVulnerabilities)}`
            );
            console.log(
                `High Priority (<70): ${chalk.yellow(dashboard.overview.highVulnerabilities)}`
            );

            // Generate recommendations
            if (dashboard.overview.criticalVulnerabilities > 0) {
                dashboard.recommendations.push(
                    'Immediate action required for repositories with critical security issues'
                );
            }
            if (dashboard.overview.vulnerableRepos > 0) {
                dashboard.recommendations.push(
                    'Review and update security policies for vulnerable repositories'
                );
            }
            ...
            if (options.save) {
                const fs = require('fs');
                const filename = `security-dashboard-${new Date().toISOString().split('T')[0]}.json`;
                fs.writeFileSync(
                    filename,
            ...
            console.error(
                chalk.red(
                    `❌ Security dashboard generation failed: ${error.message}`
                )
            );
            throw error;
        }
