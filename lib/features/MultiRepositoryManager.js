...
class MultiRepositoryManager extends RepositoryManager {
    ...
    async getOrganizationRepositories() {
        ...
        try {
            ...
            console.log(
                `⚠️  Failed to fetch organization repositories: ${error.message}`
            );
            return null;
        }
    }
    ...
    generateOrganizationRecommendations(results) {
        ...
        const iotRepos = repos.filter(r => 
            ...
            r.topics?.some(t => ['iot', 'firmware', 'embedded', 'esp32', 'sensor'].includes(t))
        );
        
        const firmwareRepos = repos.filter(r =>
            r.language === 'C++' || r.language === 'C' ||
            r.name?.toLowerCase().includes('firmware') ||
            r.topics?.some(t => ['firmware', 'embedded', 'esp32', 'esp8266', 'arduino'].includes(t))
        );
        
        const serverRepos = repos.filter(r =>
            (r.language === 'Python' || r.language === 'JavaScript') &&
            (r.name?.toLowerCase().includes('server') || 
             r.description?.toLowerCase().includes('server') ||
             r.topics?.some(t => ['server', 'backend', 'api'].includes(t)))
        );
        ...
        const securityRepos = repos.filter(r => 
            r.name?.toLowerCase().includes('security') ||
            r.topics?.some(t => ['security', 'authentication', 'encryption'].includes(t))
        );
        ...
        if (iotRepos.length > 0 && securityRepos.length === 0) {
            recommendations.push({
                priority: 'high',
                type: 'iot-security',
                message: 'No dedicated security repository found - consider creating centralized IoT security standards and tools',
            });
        }
        ...
        if (iotRepos.length > 2 && docRepos.length === 0) {
            recommendations.push({
                ...
                message: 'Multiple IoT repositories without centralized documentation - consider creating organization documentation hub',
            });
        }
