...
class MultiRepositoryManager extends RepositoryManager {
    ...
    async getRepositoriesViaGitHubApp() {
        try {
            ...
            if (allRepositories.length === 0) {
                throw new Error('No repositories found via GitHub App installation');
            }

            console.log(`✅ Found ${allRepositories.length} repositories via GitHub App`);
    ...
    async getRepositoriesViaOrgAPI() {
        ...
        console.log(`✅ Found ${allRepositories.length} repositories via Organization API`);
