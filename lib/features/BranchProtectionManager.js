...
class BranchProtectionManager extends RepositoryManager {
  async auditBranchProtection() {
    ...
    for (const branch of mainBranches) {
      ...
      const analysis = this.analyzeBranchProtection(branch.name, protection);
  ...
  async getMainBranches() {
    ...
    return branches.filter(branch => 
      mainBranchNames.includes(branch.name) || branch.name === repo.default_branch
    );
  }
  ...
  analyzeBranchProtection(branchName, protection) {
    ...
    if (protection.required_linear_history?.enabled) {
      analysis.checks.push({ name: 'Linear history required', status: true });
      analysis.score += 10;
    }
  ...
  async enforceBranchProtection(options = {}) {
    ...
    for (const branch of mainBranches) {
      try {
        await this.updateBranchProtection(branch.name, defaultProtection);
