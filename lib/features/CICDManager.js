...
class CICDManager extends RepositoryManager {
  ...
  checkWorkflowSecurity(content, analysis) {
    ...
    if (/actions\/checkout@v[34]/.test(content)) {
      checks.push({ name: 'Secure checkout action', status: true, weight: 10 });
    } else if (/actions\/checkout@v[12]/.test(content)) {
      analysis.securityIssues.push('Outdated checkout action version');
    }

    // Check for hardcoded secrets
    if (/(?:password|secret|token|key)\s*[:=]\s*['"][^'"]+['"]/i.test(content)) {
    ...
    if (/pull_request_target/.test(content)) {
      analysis.securityIssues.push('Uses pull_request_target - review for security implications');
    }

    // Check permissions
    if (/permissions:/.test(content)) {
      checks.push({ name: 'Explicit permissions set', status: true, weight: 15 });
    } else {
      analysis.recommendations.push('Set explicit permissions for better security');
    }
  ...
  checkWorkflowBestPractices(content, analysis) {
    ...
    if (/strategy:.*matrix:/s.test(content)) {
      checks.push({ name: 'Uses matrix strategy', status: true, weight: 8 });
    }
  ...
  checkWorkflowPerformance(content, analysis) {
    ...
    if (jobMatches && jobMatches.length > 1) {
      checks.push({ name: 'Multiple jobs for parallelism', status: true, weight: 8 });
    }
  ...
  async checkEssentialWorkflows(results) {
    const existingWorkflows = results.workflows.map(w => w.name.toLowerCase());
    
    const essentialWorkflows = [
      { name: 'ci', patterns: ['ci', 'test', 'build'], weight: 30 },
      { name: 'security', patterns: ['security', 'codeql', 'dependency'], weight: 20 },
      { name: 'release', patterns: ['release', 'publish', 'deploy'], weight: 15 }
    ];

    let essentialScore = 0;
    const maxEssentialScore = essentialWorkflows.reduce((sum, w) => sum + w.weight, 0);
    ...
    for (const essential of essentialWorkflows) {
      ...
      if (exists) {
        ...
        results.recommendations.push(`Consider adding ${essential.name} workflow`);
      }
    }

    results.essentialWorkflowsScore = Math.round((essentialScore / maxEssentialScore) * 100);
  }
  ...
  calculateCICDScore(results) {
    ...
    const avgWorkflowScore = workflowScores.reduce((sum, score) => sum + score, 0) / workflowScores.length;
