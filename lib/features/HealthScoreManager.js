...
class HealthScoreManager {
    ...
    async calculateHealthScore() {
        ...
        try {
            ...
            [securityAudit, branchAudit, docsAudit, cicdAudit] =
                await Promise.all([
                    this.security
                        .securityAudit()
                        .catch((err) => ({ score: 0, error: err.message })),
                    this.branches
            ...
            if (
                ...
                console.log(
                    '⚠️  API audits failed, using enhanced local auditing...'
                );
            ...
            console.log(
                '⚠️  All audits failed, using enhanced local auditing...'
            );
            this.useLocalFallback = true;
        ...
        for (const [category, data] of Object.entries(results.categories)) {
            if (!data.error && typeof data.score === 'number') {
                ...
                results.criticalIssues.push(
                    `${category}: ${data.error || 'Invalid score'}`
                );
            }
        }

        results.score =
            totalWeight > 0 ? Math.min(100, Math.round(totalWeightedScore / totalWeight)) : 0;

        // Add IoT-specific bonus scoring
        const iotBonus = await this.calculateIoTBonus();
        if (iotBonus > 0) {
            results.score = Math.min(results.score + iotBonus, 100);
            results.iotBonus = iotBonus;
        }
    ...
    async runLocalSecurityAudit() {
        try {
            ...
            return {
                score: 0,
                error: `Local security audit failed: ${error.message}`,
            };
        }
    }
    ...
    async runLocalBranchAudit() {
        try {
            const result = await this.runLocalScript(
            ...
            return {
                score: 0,
                error: `Local branch audit failed: ${error.message}`,
            };
        }
    }
    ...
    async runLocalDocumentationAudit() {
        try {
            ...
            return {
                score: 0,
                error: `Local documentation audit failed: ${error.message}`,
            };
        }
    }
    ...
    async runLocalCICDAudit() {
        try {
            ...
            return {
                score: 0,
                error: `Local CI/CD audit failed: ${error.message}`,
            };
        }
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            child.on('close', (code) => {
                if (code === 0) {
                    ...
                    reject(
                        new Error(
                            `Script failed with code ${code}: ${errorOutput}`
                        )
                    );
                }
            });
    ...
    async calculateIoTBonus() {
        try {
            const IoTManager = require('./IoTManager');
            const iotManager = new IoTManager(this.config);

            const isIoT = await iotManager.detectIoTRepository();
            if (!isIoT) return 0;

            let bonus = 0;

            // Check for IoT-specific files and give bonus points
            const iotFiles = await iotManager.checkIoTFiles();

            // Bonus for having IoT-specific configuration
            if (iotFiles.includes('platformio.ini')) bonus += 3;
            if (iotFiles.includes('arduino.ino')) bonus += 2;
            if (iotFiles.includes('CMakeLists.txt')) bonus += 2;
            if (iotFiles.includes('docker-compose.yml')) bonus += 3;

            // Bonus for IoT-specific documentation
            if (iotFiles.some((f) => f.includes('HARDWARE'))) bonus += 2;
            if (iotFiles.some((f) => f.includes('API'))) bonus += 2;

            // Bonus for having proper IoT project structure
            if (iotFiles.includes('src/') && iotFiles.includes('include/'))
                bonus += 3;
            if (iotFiles.includes('lib/')) bonus += 2;
            if (iotFiles.includes('tests/') || iotFiles.includes('test/'))
                bonus += 2;

            return Math.min(bonus, 15); // Cap at 15 bonus points
        } catch (error) {
            // If IoT detection fails, no bonus
            return 0;
        }
    }
    ...
    generateRecommendations(results) {
        ...
        for (const [category, data] of Object.entries(results.categories)) {
            if (data.score < 70) {
                if (category === 'security') {
                    ...
                    recommendations.push(
                        '🔴 CI/CD pipeline needs significant improvement'
                    );
                } else if (category === 'branchProtection') {
                    recommendations.push(
                        '🔴 Branch protection is critically lacking'
                    );
        ...
        if (this.useLocalFallback) {
            recommendations.push(
                'ℹ️  GitHub API access limited - consider configuring API token for enhanced features'
            );
        }
        ...
        recommendations.sort((a, b) => {
            const priorityA = priorityOrder.findIndex((p) =>
                a.toLowerCase().includes(p)
            );
            const priorityB = priorityOrder.findIndex((p) =>
    ...
    generateSummary(results) {
        results.summary = {
            ...
            passingCategories: Object.values(results.categories).filter(
                (c) => c.score >= 70
            ).length,
