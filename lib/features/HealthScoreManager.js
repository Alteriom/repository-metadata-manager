...
class HealthScoreManager {
    ...
    async calculateHealthScore() {
        ...
        const [securityAudit, branchAudit, docsAudit, cicdAudit] =
            await Promise.all([
                this.security
                    .securityAudit()
        ...
        for (const [category, data] of Object.entries(results.categories)) {
            if (data.score < 70) {
                recommendations.push({
                    ...
                    message: this.getCategoryRecommendation(
                        category,
                        data.score
                    ),
        ...
        const messages = {
            ...
            cicd:
                score < 40
                    ...
                    : 'CI/CD practices could be enhanced',
            branchProtection:
                score < 40
                    ? 'Branch protection is critically lacking'
                    : 'Branch protection rules need strengthening',
        };
        return messages[category] || 'Needs improvement';
    }

    getCategoryActions(category, _data) {
        const actions = {
            security: [
