...
class BranchProtectionChecker extends Checker {
  ...
  async check(context) {
    ...
    if (actual.approvals >= (desired.requiredApprovals || 0)) score += 12;
    else this._finding(
      findings,
      'bp-012',
      'high',
      `Required approvals are ${actual.approvals}; policy requires ${desired.requiredApprovals}`,
      `Require at least ${desired.requiredApprovals} approving review(s)`,
    );
