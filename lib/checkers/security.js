  ...
  async check(context) {
    ...
    if (context.packageJson && context.fileExists('package-lock.json')) {
      const auditResult = await context.cache.getOrSet('npm-audit', async () => {
        ...
        try {
          // Candidate npm configuration may control the registry, so the audit
          // subprocess receives only a minimal, credential-free environment.
          // CLI include flags override project-level omit settings for every
          // dependency class npm permits callers to omit.
          const output = childProcess.execSync(
            'npm audit --json --ignore-scripts --include=dev --include=optional --include=peer', {
