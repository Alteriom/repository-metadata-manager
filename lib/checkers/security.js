...
function npmAuditCommand(packageJson) {
  ...
  const hasWorkspaces = Array.isArray(workspaces)
    ? workspaces.length > 0
    : Boolean(workspaces && Array.isArray(workspaces.packages) && workspaces.packages.length > 0);
...
function prepareNpmAuditProject(context, auditHome) {
  ...
  const relativeManifests = new Set();
  for (const packagePath of Object.keys(lock.packages || {})) {
    if (!packagePath || packagePath.split('/').includes('node_modules')) continue;
    relativeManifests.add(`${packagePath}/package.json`);
  }

  const workspaceConfig = context.packageJson.workspaces;
  const workspacePatterns = Array.isArray(workspaceConfig)
    ? workspaceConfig
    : workspaceConfig?.packages;
  for (const pattern of workspacePatterns || []) {
    if (typeof pattern !== 'string' || !pattern || pattern.startsWith('!')) continue;
    const normalized = pattern.replaceAll('\\', '/').replace(/\/+$/, '');
    if (!normalized || path.posix.isAbsolute(normalized) ||
        normalized.split('/').includes('..')) {
      throw new Error('package.json contains a workspace outside the audit root');
    }
    for (const match of fs.globSync(`${normalized}/package.json`, {
      cwd: context.projectRoot,
    })) {
      const relativeManifest = match.replaceAll('\\', '/');
      const segments = relativeManifest.split('/');
      if (!segments.includes('node_modules') && !segments.includes('.git')) {
        relativeManifests.add(relativeManifest);
      }
    }
  }

  for (const relativeManifest of relativeManifests) {
