...
class Report {
  static aggregate(checkResults, checkerConfigs = {}, options = {}) {
    ...
    const gateResults = Report.evaluateGates(
      { score, bySeverity, checkers },
      policy.gates || {},
      { checkerScope: options.checkerScope },
    );
  ...
  static evaluateGates({ score, bySeverity, checkers }, gates, options = {}) {
    const results = [];
    const checkerScope = Array.isArray(options.checkerScope) ? new Set(options.checkerScope) : null;
    ...
    for (const checker of gates.requireVerifiedCheckers || []) {
      if (checkerScope && !checkerScope.has(checker)) continue;
